...
class TypeList{
    ...
    func getNonEmptyFolderType() -> [Type]{
        var nonEmptyFolderType = [Type]()
        
        for i in self.folderTypes{
            if i.getFolderCount() != 0{
                nonEmptyFolderType.append(i)
            }
        }
        return nonEmptyFolderType
    }
...
//        self.addFolderTypeToRealm(typename: "Plants")
