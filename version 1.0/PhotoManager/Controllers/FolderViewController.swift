...
//        CollectionViewService.setupLayout(collectionView: self.collectionView, controller: self, cellPadding: 0, numberOfColumns: 3)
    ...
    @IBAction func deleteFolder(_ sender: UIBarButtonItem) {
        CollectionViewService.deleteFoldersFromRealm(folders: self.folderType, controller: self, collectionView: self.collectionView, addButton:self.addButton, updateButton: self.updateButton, deleteButton: self.deleteButton, textFieldDelegate: self)
    }
...
extension FolderViewController: UICollectionViewDataSource{
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        ...
        if (self.folderType.getFolderArray().count == 0) {
            
            if searchBar.text != "" {
                self.collectionView.setEmptyMessage("No data found")
            }else{
                self.collectionView.setEmptyMessage("Please create your own folder!")
            }
        } else {
