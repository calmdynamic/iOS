...
class FolderTypeViewController: UIViewController, UISearchBarDelegate, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        SearchBarService.setSearchBarProperty(searchBar: self.searchBar, searchBarDelegate: self, placeholder: "Search Folder Types", bookmark: true)
    ...
    override func setEditing(_ editing: Bool, animated: Bool) {
        ...
        SearchBarService.searchBarChangeWhenSetEditng(searchBar: self.searchBar, editing: editing, bookmark: true)
...
extension FolderTypeViewController: UICollectionViewDataSource{
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        ...
        if (self.folderTypes.getFolderTypes().count == 0) {
            
            if searchBar.text != "" {
                self.collectionView.setEmptyMessage("No data found")
            }else{
                self.collectionView.setEmptyMessage("Please create your own folder type(s)!")
            }
        } else {
...
//
        if cell.folderTypeLabel.text == "Default"{
            ...
            let numOfImages = self.folderTypes.getOneFolderTypeByIndedx(index: indexPath.item).getFolders()[0].getImageCount()
            
            cell.numOfFolders.text = numOfImages == 0 ? "Empty" : "\(numOfImages)"
        }else{
...
extension FolderTypeViewController: UICollectionViewDelegate{
    ...
    func searchBarTextDidBeginEditing(_ searchBar: UISearchBar) {
        SearchBarService.searchBarButtonChangeWhenIsEditing(viewController: self, collectionView: self.collectionView, searchBar: self.searchBar, tapGesture: self.tapGesture, isBeginEditing: true, bookmark: true)
    }
    
    //a function to change all button status when finishing editing on search bar
    func searchBarTextDidEndEditing(_ searchBar: UISearchBar) {
        SearchBarService.searchBarButtonChangeWhenIsEditing(viewController: self, collectionView: self.collectionView, searchBar: self.searchBar, tapGesture: self.tapGesture, isBeginEditing: false, bookmark: true)
    }
